...
# Micro Servicio AhlMsDetallePeriodoHlJava
<p align="center">
...
Servicio que permite validar información AhlMsDetallePeriodoHlJava completar por cada desarrollador
A continuacion se relacionan cada uno de los metodos que presenta este servicio:

 ````
    http post
    Hearder Required: [X_RQUID; X_CHANNEL; X_GOV_ISSUE_IDENT_TYPE; X_IDENT_SERIAL_NUM; X_COMPANY_ID; X_IP_ADDR; Authorization;]
    Request: 
 ````
 ````
    Response: 
...
```
    ...
    docker image rm azuepvgoydvpsptacr.azurecr.io/ahl-ms-detalle-periodo-hl-java:latest -f
    
    Creaciòn Image
    docker build -t azuepvgoydvpsptacr.azurecr.io/ahl-ms-detalle-periodo-hl-java:latest --no-cache .  
...
```
   docker push azuepvgoydvpsptacr.azurecr.io/ahl-ms-detalle-periodo-hl-java:latest
   az acr repository list --name azuepvgoydvpsptacr.azurecr.io
   az acr repository show-tags --name azuepvgoydvpsptacr.azurecr.io --repository AhlMsDetallePeriodoHlJava
```
...
```
    Comando 0 => kubectl delete deployment ahl-ms-detalle-periodo-hl-java-deployment  -n  NAMESPACE_AKS

    Comando 1 => sed -i 's/REPLICASET/1/g' ./k8s/ahl-ms-detalle-periodo-hl-java-deployment.yaml | sed -i 's/NAME_ARTIFACT/ahl-ms-detalle-periodo-hl-java/g' ./k8s/ahl-ms-detalle-periodo-hl-java-deployment.yaml | sed -i 's/VERSION/latest/g' ./k8s/ahl-ms-detalle-periodo-hl-java-deployment.yaml | sed -i 's/GENERACION/20221228102900/g' ./k8s/ahl-ms-detalle-periodo-hl-java-deployment.yaml | kubectl apply -f ./k8s/ahl-ms-detalle-periodo-hl-java-deployment.yaml
